...
async function main() {
  ...
  const site1 = await prisma.heritageSite.create({
    data: {
      ...
      images: {
        create: [
          { image_data: Buffer.from('dummy_image_data_1') },
          { image_data: Buffer.from('dummy_image_data_2') },
        ],
      },
      stories: {
        create: [
  ...
  const museum1 = await prisma.museum.create({
    data: {
      name: 'Patan Museum',
      description: 'Traditional sacred art of Nepal in an architectural setting.',
      opening_hours: '10:00 AM - 5:00 PM',
      gps_coordinates: '27.6744° N, 85.3245° E',
      images: {
        create: [
          { image_data: Buffer.from('museum_image_1') },
        ],
      },
    },
  });
