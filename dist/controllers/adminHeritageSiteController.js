...
export const addHeritageSite = async (req, res) => {
    const { name, location, description, photo_url, gps_coordinates } = req.body;
    const file = req.file;
    ...
    if (!name || !location || !description || !photo_url || !gps_coordinates) {
        return res.status(400).json({
            message: 'Missing required fields: name, location, description, photo_url, and gps_coordinates are required',
        });
    }
    try {
