...
export default function HomeScreen() {
  ...
  const heritageSliderRef = useRef<ScrollView>(null);
  const museumSliderRef = useRef<ScrollView>(null);
  const heritageIndexRef = useRef(0);
  const museumIndexRef = useRef(0);
  const sliderCardWidth = Dimensions.get("window").width - 40;
  const sliderGap = 16;
  const sliderStep = sliderCardWidth + sliderGap;
  ...
  useEffect(() => {
    if (heritageSites.length <= 1) return;

    const timer = setInterval(() => {
      const next = (heritageIndexRef.current + 1) % heritageSites.length;
      heritageIndexRef.current = next;
      heritageSliderRef.current?.scrollTo({
        x: next * sliderStep,
        animated: true,
      });
    }, 3500);
  ...
  }, [heritageSites.length, sliderStep]);

  useEffect(() => {
    if (museums.length <= 1) return;

    const timer = setInterval(() => {
      const next = (museumIndexRef.current + 1) % museums.length;
      museumIndexRef.current = next;
      museumSliderRef.current?.scrollTo({
        x: next * sliderStep,
        animated: true,
      });
    }, 3500);
    ...
    return () => clearInterval(timer);
  }, [museums.length, sliderStep]);

  const handleHeritageScrollEnd = (
    event: NativeSyntheticEvent<NativeScrollEvent>,
  ) => {
    const x = event.nativeEvent.contentOffset.x;
    heritageIndexRef.current = Math.round(x / sliderStep);
  };

  const handleMuseumScrollEnd = (
    event: NativeSyntheticEvent<NativeScrollEvent>,
  ) => {
    const x = event.nativeEvent.contentOffset.x;
    museumIndexRef.current = Math.round(x / sliderStep);
  };
      ...
      >
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            <Text style={styles.greeting} testID="homeGreeting">
              🙏 {t("greeting")}
            </Text>
        ...
        <View style={styles.heroSection}>
          ...
          <Text style={styles.heroDescription}>{t("discoverDesc")}</Text>
        </View>
          ...
          <>
            ...
            <View style={styles.section}>
              ...
              <ScrollView
                ...
                onMomentumScrollEnd={handleHeritageScrollEnd}
              >
                {heritageSites?.map(
                  (site, index) =>
                    site && (
                      <TouchableOpacity
                        key={
                          site.site_id
                            ? `heritage-${site.site_id}`
                            : `heritage-idx-${index}`
                        }
                        style={[styles.premiumCard, { width: sliderCardWidth }]}
                        activeOpacity={0.9}
                        onPress={navigateToHeritageSites}
                      >
            ...
            <View style={styles.section}>
              ...
              <ScrollView
                ...
                showsHorizontalScrollIndicator={false}
                contentContainerStyle={styles.cardsContainer}
                onMomentumScrollEnd={handleMuseumScrollEnd}
              >
                {museums?.map(
                  (museum, index) =>
                    museum && (
